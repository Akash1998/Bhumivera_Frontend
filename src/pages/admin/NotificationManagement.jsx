// anritvox-frontend/src/pages/admin/NotificationManagement.jsx
...
export default function NotificationManagement() {
  ...
  const [formData, setFormData] = useState({
    ...
    user_id: '', // Leave blank for global
  });
  ...
  const handleSend = async (e) => {
    e.preventDefault();
    try {
      // Ensure user_id is null if left empty to trigger global notification
      const payload = {
        ...formData,
        user_id: formData.user_id.trim() === '' ? null : formData.user_id
      };
      
      await api.post('/api/notifications/admin/create', payload);
              ...
              {loading ? (
                ...
                filteredNotifications.map((notif) => (
                  <tr key={notif.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                    ...
                    <td className="px-6 py-4">
                      {notif.user_id ? (
                        <span className="bg-slate-800 text-slate-300 px-2 py-1 rounded-md text-xs font-mono">User: {notif.user_id}</span>
                      ) : (
                        <span className="bg-emerald-500/10 text-emerald-400 px-2 py-1 rounded-md text-xs font-bold">Global Broadcast</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl w-full max-w-lg relative">
            ...
            <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
              <Send size={20} className="text-emerald-500" /> Send Notification
            </h3>
            ...
            <form onSubmit={handleSend} className="space-y-4">
              <div>
                ...
                <input 
                  type="text" 
                  required
                  placeholder="e.g. Flash Sale Starting Soon!"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select 
                    className="w-full bg-slate-950 border border-slate-800 text-white rounded-lg px-4 py-2 focus:border-emerald-500 focus:outline-none appearance-none"
                    value={formData.type}
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-950 border border-slate-800 text-white rounded-lg px-4 py-2 focus:border-emerald-500 focus:outline-none placeholder:text-slate-600 font-mono text-sm"
              ...
              <button 
                type="submit" 
                className="w-full bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold py-3 rounded-xl transition-all mt-6 flex justify-center items-center gap-2"
              >
