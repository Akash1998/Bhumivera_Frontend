...
export default function Warehouse() {
  ...
  const handleLogout = useCallback(() => {
    ['token', 'ms_token', 'warehouseToken', 'user', 'warehouseUser'].forEach(k => localStorage.removeItem(k));
  ...
  const init = useCallback(async () => {
    ...
    try {
      const t = localStorage.getItem('token') || localStorage.getItem('ms_token') || localStorage.getItem('warehouseToken');
      if (!t || t === 'null' || t === 'undefined') { handleLogout(); return; }
      const accessRes = await api.get('/warehouse/check-access');
      if (!accessRes.data.hasAccess && !accessRes.data.isAdmin) { setAccessDenied(true); setLoading(false); return; }
  ...
  const handleRestore = async e => {
    ...
    r.onload = async ev => {
      try { await api.post('/warehouse/restore-legacy', { backupData: JSON.parse(ev.target.result) }); showToast?.('Legacy Ledger Synced', 'success'); } 
  ...
  const handleSubmit = async e => {
    ...
    try {
      await api.post(formData.isWalkin ? '/warehouse/walkin-sale' : '/warehouse/log-sale', { product_name: formData.product_name, quantity: formData.quantity, sale_price: parseFloat(formData.sale_price) });
      showToast?.('Transaction secured.', 'success');
      setFormData({ product_name: '', quantity: 1, sale_price: '', isWalkin: formData.isWalkin });
    } catch (e) { showToast?.(e?.response?.data?.message || 'Log failed.', 'error'); } 
  ...
  if (accessDenied || error) return <div className="min-h-screen flex flex-col items-center justify-center bg-[#020617] text-center px-4"><AlertTriangle className="w-12 h-12 text-rose-500 mb-4" /><h2 className="text-2xl font-black text-white uppercase tracking-tight mb-2">{error ? 'Connection Error' : 'Access Denied'}</h2><p className="text-slate-500 text-sm mb-6 uppercase font-bold">{error || 'You lack distributor clearance for this portal.'}</p><button onClick={handleLogout} className="px-6 py-3 bg-rose-500/10 text-rose-500 border border-rose-500/30 rounded-xl text-xs font-black uppercase hover:bg-rose-500 hover:text-white transition-all">Return to Login</button></div>;
