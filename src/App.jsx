...
export default function Warehouse() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Log sale error:', e);
  ...
  if (accessDenied) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#020617] text-center px-4">
      ...
      <p className="text-slate-500 text-sm mb-6">You lack distributor clearance. Contact admin.</p>
      <button onClick={handleLogout} className="px-4 py-2 bg-rose-500/10 text-rose-500 border border-rose-500/30 rounded-xl text-sm font-black uppercase hover:bg-rose-500 hover:text-white transition-all">
        Return to Login
      </button>
    </div>
  );
  ...
  if (error) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#020617] text-center px-4">
      ...
      <h2 className="text-xl font-black text-white uppercase tracking-tight mb-2">Connection Error</h2>
      <p className="text-slate-400 text-sm mb-6">{error}</p>
      <button onClick={init} className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded-xl text-sm font-black uppercase hover:bg-emerald-500 hover:text-slate-950 transition-all">
        ...
        <div className="flex items-center justify-between mb-8 pb-6 border-b border-slate-800">
          <div>
            <h1 className="text-2xl font-black text-white uppercase tracking-tight flex items-center gap-2">
              ...
              Local <span className="text-emerald-500">Warehouse</span>
            </h1>
            <p id="store-name" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">{storeName}</p>
          </div>
          <button onClick={handleLogout} className="p-2.5 bg-rose-500/10 text-rose-500 border border-rose-500/20 rounded-xl hover:bg-rose-500 hover:text-white transition-all shadow-md flex items-center gap-2 text-xs font-black uppercase">
        ...
        <div className="bg-[#0f172a] border border-slate-800 rounded-2xl p-6 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              ...
              className="w-full py-3 bg-emerald-500 text-slate-950 font-black uppercase tracking-widest rounded-xl hover:bg-emerald-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-emerald-500/20"
            >
              {isSubmitting ? 'Syncing to Ledger...' : 'Log Transaction to Ledger'}
            </button>
          </form>
        </div>
      </div>
